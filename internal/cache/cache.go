...
func Open(path string) (*Cache, error) {
	...
	cache := &Cache{db: db}
	if err := cache.migrate(context.Background()); err != nil { db.Close(); return nil, err }
	return cache, nil
}
...
ON CONFLICT(id) DO UPDATE SET name=excluded.name, username=excluded.username`, user.ID, user.Name, user.Username); err != nil { return err }
	}
	for _, event := range page.Data {
		if event.EventType != "MessageCreate" || event.ConversationID == "" { continue }
...
ON CONFLICT(id) DO UPDATE SET
 ...
 updated_at=MAX(conversations.updated_at,excluded.updated_at)`, event.ConversationID, title, created); err != nil { return err }
		if _, err := tx.ExecContext(ctx, `INSERT INTO messages(id,conversation_id,sender_id,text,created_at) VALUES(?,?,?,?,?)
ON CONFLICT(id) DO UPDATE SET text=excluded.text`, event.ID, event.ConversationID, event.SenderID, event.Text, created); err != nil { return err }
	}
	return tx.Commit()
}
...
func conversationTitle(id string, participants []string, selfID string, users map[string]api.User) string {
	...
	if len(ids) == 0 && strings.Contains(id, "-") { ids = strings.Split(id, "-") }
	var names []string
	...
	sort.Strings(names)
	if len(names) > 0 { return strings.Join(names, ", ") }
	if strings.Contains(id, "-") { return "Direct message" }
...
GROUP BY c.id,c.title,latest.text,c.updated_at ORDER BY c.updated_at DESC`, like, like, like)
	...
	for rows.Next() {
		...
		var updated string
		if err := rows.Scan(&item.ID,&item.Title,&item.Preview,&updated,&item.UnreadCount); err != nil { return nil, err }
...
WHERE m.conversation_id=? ORDER BY m.created_at`, conversationID)
	...
	for rows.Next() {
		var item Message
		var created string
		if err := rows.Scan(&item.ID,&item.ConversationID,&item.SenderID,&item.SenderName,&item.Text,&created); err != nil { return nil, err }
...
func (c *Cache) SaveSent(ctx context.Context, conversationID,eventID,senderID,text string,sentAt time.Time) error {
	...
	if err := c.SavePage(ctx,page,senderID); err != nil { return fmt.Errorf("saving sent message: %w",err) }
